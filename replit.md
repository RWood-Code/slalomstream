...
- **Cloudflare Tunnel ("Go Online")**: Admin > Network panel has a single toggle that spawns a `cloudflared` sidecar (quick tunnel — no account required, assigns a random `*.trycloudflare.com` URL; or named tunnel with a user-provided token). When active: `connection_mode='tunnel'` + `public_url` are written to `app_settings`; QR codes on the Recording page update automatically; write-route protection middleware (`requireAdminIfPublic`) blocks POST/PUT/DELETE on tournament/skier/pass/judge routes unless a valid `X-Admin-Token` header is present. GET routes, judge score submission, and settings routes remain open. The cloudflared binary itself must be placed in `src-tauri/binaries/` (see Task #21).
