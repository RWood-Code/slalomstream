...
# ── Stage PGlite (offline PostgreSQL — no internet required) ──────────────────
echo ""
echo "━━━ Step 4: Stage PGlite (offline database) ━━━━"
mkdir -p "$STAGE_DIR/pglite"

# Find the PGlite package in the pnpm store
PGLITE_SRC=$(find "$WORKSPACE_ROOT/node_modules/.pnpm" \
  -maxdepth 4 -type d \
  -path "*@electric-sql+pglite*" \
  -name "pglite" \
  2>/dev/null | grep "@electric-sql/pglite$" | head -1)

if [ -z "$PGLITE_SRC" ]; then
  echo "ERROR: @electric-sql/pglite not found in pnpm store."
  echo "  Run: pnpm add @electric-sql/pglite --filter @workspace/db"
  exit 1
fi

echo "  Source: $PGLITE_SRC"
cp -r "$PGLITE_SRC/." "$STAGE_DIR/pglite/"
PGLITE_SIZE=$(du -sh "$STAGE_DIR/pglite" | cut -f1)
echo "  PGlite staged (${PGLITE_SIZE})"

# ── Compile installer ──────────────────────────────────────────────────────────
echo ""
echo "━━━ Step 5: Compile installer ━━━━━━━━━━━━━━━━━"
cd "$SCRIPT_DIR"
