...
# ── Generate NSIS config header (pre-fills DATABASE_URL in installer UI) ──────
echo ""
echo "━━━ Step 4: Generate installer config ━━━━━━━━━"
cd "$SCRIPT_DIR"
node -e "
const fs = require('fs');
const url = process.env.DATABASE_URL || '';
// Do NOT pre-fill internal Replit URLs (helium is Replit-only, unreachable from Windows)
const isInternal = /@helium\b/.test(url) || url === '';
const prefill = isInternal ? '' : url;
if (isInternal && url) console.log('  Skipping pre-fill: @helium URL is Replit-internal only');
// NSIS string escaping: \$ -> \$\$  and \" -> \$\\\"
const escaped = prefill.replace(/\\\$/g, '\$\$').replace(/\"/g, '\$\\\"');
fs.writeFileSync('preconfig.nsh',
  '; Auto-generated by build-installer.sh — do not commit (may contain credentials)\n' +
  '!define DEFAULT_DATABASE_URL \"' + escaped + '\"\n'
);
console.log('  DATABASE_URL pre-fill:', prefill ? 'YES (' + prefill.substring(0, 40) + '...)' : 'NO — user must enter Neon/Supabase URL manually');
"
