...
// NSIS string escaping: \$ -> \$\$  (avoid NSIS variable expansion)
//                       \" -> \$\\\"  (escape double quotes)
const escaped = url.replace(/\\\$/g, '\$\$').replace(/\"/g, '\$\\\"');
fs.writeFileSync('preconfig.nsh',
  '; Auto-generated by build-installer.sh - do not edit or commit\n' +
  '!define DEFAULT_DATABASE_URL \"' + escaped + '\"\n'
);
console.log('  DATABASE_URL pre-fill:', url ? 'YES (' + url.substring(0, 30) + '...)' : 'NO (empty — user must enter manually)');
"
